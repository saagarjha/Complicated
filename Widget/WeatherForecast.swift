...
struct WeatherForecast: TimelineEntry {
	...
	static let sample = WeatherForecast(
		date: .now,
		hourly: [
			HourForecast(date: Date().addingTimeInterval(0 * 60 * 60), temperature: .init(value: -20, unit: .celsius), condition: "sun.max.fill"),
			HourForecast(date: Date().addingTimeInterval(1 * 60 * 60), temperature: .init(value: -10, unit: .celsius), condition: "cloud.sun.fill"),
			HourForecast(date: Date().addingTimeInterval(2 * 60 * 60), temperature: .init(value: 0, unit: .celsius), condition: "cloud.sun.rain.fill"),
			HourForecast(date: Date().addingTimeInterval(3 * 60 * 60), temperature: .init(value: 10, unit: .celsius), condition: "cloud.sun.bolt.fill"),
			HourForecast(date: Date().addingTimeInterval(4 * 60 * 60), temperature: .init(value: 20, unit: .celsius), condition: "moon.stars.fill"),
		],
